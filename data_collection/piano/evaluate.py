"""
피아노 주법 분류 모델 평가 스크립트
inst-ai/data_collection/piano/evaluate.py

사용법:
  python evaluate.py --model crnn
...
def load_val_dataset(model_type: str = 'crnn') -> DataLoader:
    x_path = os.path.join(PROCESSED_DIR,
                          'X_raw.npy' if model_type == 'panns' else 'X.npy')
    X = np.load(x_path)
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    ...
    for c in range(len(LABELS)):
        ...
        metrics[LABELS[c]] = {
            ...
            'f1': f1, 'support': int(np.sum(y_true == c)),
        }
...
def print_metrics(metrics: dict, model_type: str) -> None:
    ...
    print(f"{'accuracy':10s} {'':>10} {'':>10} "
          ...
          f"{sum(metrics[l]['support'] for l in LABELS):>10d}")
...
def plot_comparison_table(results: dict) -> None:
    ...
    for m_key, m_label in zip(metrics_keys, metric_labels):
        row = [m_label] + [f"{results[mt][m_key]:.3f}" for mt in models]
    ...
    table.scale(1.2, 2)
    ax.set_title('CRNN vs PANNs Fine-tuning', pad=20)
...
def main():
    parser = argparse.ArgumentParser(description="피아노 주법 분류 모델 평가")
    parser.add_argument('--model', required=True,
                        choices=['crnn', 'panns', 'both'])
